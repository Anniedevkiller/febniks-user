...
export function FoodCard({ id, name, description, price, image, onAdd, onClick }: FoodCardProps) {
  return (
    <motion.div 
      whileHover={{ scale: 1.02 }}
      whileTap={{ scale: 0.98 }}
      transition={{ type: "spring", stiffness: 400, damping: 17 }}
      className="group relative bg-white cursor-pointer transition-all duration-300"
      onClick={onClick}
    >
